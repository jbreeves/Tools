...
def rate_applications(json_file_path):
    ...
    for app_id, app_data in applications.items():
        ...
        results.append({
            ...
            'owner': app_data.get('appOwner'),
            'security_advisor': app_data.get('securityAdvisor'),
            'total_score': score,
            'rating': rating,
...
def print_report(results):
    ...
    for r in results:
        ...
        if r['data_classification'] == 'Red - Highly Confidential': high_conf_data_count += 1
...
def print_app_details(app):
    """Print detailed view of a single application."""
    print("\n" + "=" * 80)
    print(f"APPLICATION DETAILS: {app['app_name']} ({app['app_id']})")
    print("=" * 80)
    
    print(f"{'Portfolio:':<25} {app.get('portfolio', 'N/A')}")
    print(f"{'Owner:':<25} {app.get('owner', 'N/A')}")
    print(f"{'Security Advisor:':<25} {app.get('security_advisor', 'N/A')}")
    print("-" * 80)
    
    print(f"{'Total Score:':<25} {app['total_score']}")
    print(f"{'Rating:':<25} {app['rating']}")
    print("-" * 80)
    
    # Impact Breakdown
    print("\nIMPACT FACTORS:")
    print(f"  {'Factor':<30} {'Score'}")
    print(f"  {'-'*30} {'-'*5}")
    impact_factors = app['score_breakdown']['impact_factors']
    for factor, score in impact_factors.items():
        print(f"  {factor:<30} {score}")
    print(f"  {'TOTAL IMPACT:':<30} {app['score_breakdown']['impact_score']}")

    # Likelihood Breakdown
    print("\nLIKELIHOOD FACTORS:")
    print(f"  {'Factor':<30} {'Score'}")
    print(f"  {'-'*30} {'-'*5}")
    likelihood_factors = app['score_breakdown']['likelihood_factors']
    for factor, score in likelihood_factors.items():
        print(f"  {factor:<30} {score}")
    print(f"  {'TOTAL LIKELIHOOD:':<30} {app['score_breakdown']['likelihood_score']}")
    print("=" * 80 + "\n")

def interactive_search_loop(results):
    """Run interactive search for applications."""
    print("\n" + "*" * 80)
    print("INTERACTIVE SEARCH MODE")
    print("Type an Application Name or ID to search. Type 'exit' or 'quit' to stop.")
    print("*" * 80)
    ...
    while True:
        ...
        for app in results:
            ...
            if (query.lower() in app['app_id'].lower()) or \
               (query.lower() in app['app_name'].lower()):
                matches.append(app)
        
        if not matches:
            print("  No applications found matching that query.")
        elif len(matches) == 1:
            print_app_details(matches[0])
        else:
            print(f"\n  Found {len(matches)} matches. Please be more specific or check the list:")
            for m in matches:
                print(f"  - {m['app_id']}: {m['app_name']}")

if __name__ == "__main__":
    results = rate_applications('applications.json')
    if results:
        print_report(results)
        interactive_search_loop(results)